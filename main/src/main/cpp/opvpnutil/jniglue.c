...
void Java_de_blinkt_openvpn_utils_NativeUtils_jniclose(JNIEnv *env,jclass jo, jint fd)
{
	int ret = close(fd);
}

//! Hack to get the current installed ABI of the libraries. See also https://github.com/schwabe/ics-openvpn/issues/391
jstring Java_de_blinkt_openvpn_utils_NativeUtils_getJNIAPI(JNIEnv *env, jclass jo)
{
    return (*env)->NewStringUTF(env, TARGET_ARCH_ABI);
}

jstring Java_de_blinkt_openvpn_utils_NativeUtils_getOpenVPNGitVersion(JNIEnv *env, jclass jo)
{
    return (*env)->NewStringUTF(env, OPENVPN_GIT_REVISION);
}
