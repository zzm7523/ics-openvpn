...
internal class AppViewHolder(var rootView: View) : RecyclerView.ViewHolder(rootView) {
    ...
    companion object {
        fun create(inflater: LayoutInflater, parent: ViewGroup): AppViewHolder {
            val view = inflater.inflate(R.layout.allowed_app_item, parent, false)
        ...
        fun createSettingsHolder(inflater: LayoutInflater, parent: ViewGroup): AppViewHolder {
            val settingsView = inflater.inflate(R.layout.allowed_app_settings, parent, false)
...
internal class PackageAdapter(c: Context, v: VpnProfile) : RecyclerView.Adapter<AppViewHolder>(), Filterable, CompoundButton.OnCheckedChangeListener {
    ...
    private fun bindSettingsView(holder: AppViewHolder) {
        ...
        changeDisallowText(allowTextView, mProfile.isAllowedAppsVpnAreDisallowed)
        vpnOnDefaultSwitch.isChecked = mProfile.isAllowedAppsVpnAreDisallowed
        vpnOnDefaultSwitch.setOnCheckedChangeListener { _, isChecked ->
            mProfile.isAllowedAppsVpnAreDisallowed = isChecked
            notifyDataSetChanged()
        }

        val vpnAllowBypassSwitch = settingsView.findViewById<View>(R.id.allow_bypass) as Switch
        vpnAllowBypassSwitch.setOnCheckedChangeListener { _, isChecked -> mProfile.isAllowAppVpnBypass = isChecked }
        vpnAllowBypassSwitch.isChecked = mProfile.isAllowAppVpnBypass
    }

    fun bindViewApp(position: Int, viewHolder: AppViewHolder) {
        viewHolder.appInfo = mFilteredData[position]
        ...
        viewHolder.checkBox.isChecked = mProfile.allowedAppsVpn.contains(mInfo.packageName)
    }

    override fun onCheckedChanged(buttonView: CompoundButton, isChecked: Boolean) {
        val packageName = buttonView.tag as String
        if (isChecked) {
            mProfile.allowedAppsVpn.add(packageName)
        } else {
            mProfile.allowedAppsVpn.remove(packageName)
        }
    }
