...
public class FileSelectLayout extends LinearLayout implements OnClickListener {
    ...
    private FilePickerUtils.FileType fileType;
    ...
    public void parseResponse(Intent data, Context c) {
        try {
            String newData = FilePickerUtils.getFilePickerResult(fileType, data, c);
            if (newData != null) {
                setData(newData, c);
            }
    ...
    public void setCaller(FileSelectCallback fragment, int i, FilePickerUtils.FileType ft) {
        mTaskId = i;
        mFragment = fragment;
        fileType = ft;
    }
    ...
    public void onClick(View v) {
        if (v == mSelectButton) {
            Intent startFilePicker = FilePickerUtils.getFilePickerIntent(getContext(), fileType);
